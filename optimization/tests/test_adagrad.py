...
def test_AdaGrad_quadratic():
    assert np.allclose(AdaGrad(quad1).minimize()[0], quad1.x_star(), rtol=0.1)
    assert np.allclose(AdaGrad(quad2).minimize()[0], quad2.x_star(), rtol=0.1)


def test_AdaGrad_Rosenbrock():
    obj = Rosenbrock()
    assert np.allclose(AdaGrad(obj).minimize()[0], obj.x_star(), rtol=0.1)
