...
def test_Adam_standard_momentum_quadratic():
    x, _ = Adam(quad1, momentum_type='standard').minimize()
    assert np.allclose(x, quad1.x_star(), rtol=0.1)

    x, _ = Adam(quad2, momentum_type='standard').minimize()
    assert np.allclose(x, quad2.x_star(), rtol=0.01)


def test_Adam_standard_momentum_Rosenbrock():
    obj = Rosenbrock()
    x, _ = Adam(obj, momentum_type='standard', max_iter=2000).minimize()
