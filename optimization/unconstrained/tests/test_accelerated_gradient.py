...
def test_Rosenbrock():
    ...
    assert np.allclose(AcceleratedGradient(obj, momentum_type='standard').minimize()[0], obj.x_star(), rtol=0.1)
    assert np.allclose(AcceleratedGradient(obj, momentum_type='nesterov').minimize()[0], obj.x_star(), rtol=0.1)
