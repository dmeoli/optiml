...
\begin{figure}[h!]
	\centering
  	...
  	\caption{$\protect \mathcal{L}_1$-SVR with different optimization steps}
...
In order to relax the constraints in the \emph{Wolfe dual} formulation~\eqref{eq:svr_min_wolfe_dual} we define the problem as a \emph{Lagrangian dual} relaxation by embedding them into objective function, so we need to allocate the Lagrange multipliers $\mu, \lambda_+, \lambda_- \geq 0$:
...
\begin{figure}[h!]
	\centering
  	...
  	\caption{$\protect \mathcal{L}_2$-SVR with different optimization steps}
...
In order to relax the constraints in the $\mathcal{L}_2$-SVR \emph{Wolfe dual} formulation~\eqref{eq:wolfe_dual_l2_svr} we define the problem as a \emph{Lagrangian dual} relaxation by embedding them into objective function, so we need to allocate the Lagrange multipliers $\mu, \lambda \geq 0$:
