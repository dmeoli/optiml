...
We say that a function $f: \Re^m \rightarrow \Re$ is L-Lipschitz gradient continuous, i.e., L-smooth, if $f$ is differentiable and:
$$
...
Finally, we say that a function $f: \Re^m \rightarrow \Re$ is locally L-Lipschitz gradient continuous, i.e., locally L-smooth, if for every $x$ in $\Re^m$ there exists a neighborhood $U$ of $x$ such that $f$ restricted to $U$ is L-Lipschitz gradient continuous.
\end{definition}
...
\begin{theorem}[\href{https://erwanscornet.github.io/teaching/Optimization.pdf}{Stochastic Gradient Descent convergence for convex functions}] \label{thm:cvx_sgd_convergence}
...
\begin{theorem}[\href{https://erwanscornet.github.io/teaching/Optimization.pdf}{Stochastic Gradient Descent convergence for strongly convex functions}] \label{thm:str_cvx_sgd_convergence}
...
\begin{theorem}[\href{http://www.princeton.edu/~yc5/ele522_optimization/lectures/subgradient_methods.pdf}{Subgradient Descent convergence for convex functions with Polyak's stepsize}] \label{thm:cvx_polyak_subgd_convergence}
...
\begin{theorem}[\href{http://www.princeton.edu/~yc5/ele522_optimization/lectures/subgradient_methods.pdf}{Subgradient Descent convergence for convex functions}] \label{thm:cvx_subgd_convergence}
...
\begin{theorem}[\href{http://www.princeton.edu/~yc5/ele522_optimization/lectures/subgradient_methods.pdf}{Subgradient Descent convergence for strongly convex functions}] \label{thm:str_cvx_subgd_convergence}
...
\begin{theorem}[\href{https://erwanscornet.github.io/teaching/Optimization.pdf}{Gradient Descent convergence for convex functions}] \label{thm:cvx_gd_convergence}
...
\begin{theorem}[\href{https://erwanscornet.github.io/teaching/Optimization.pdf}{Gradient Descent convergence for strongly convex functions}] \label{thm:str_cvx_gd_convergence}
...
\begin{theorem}[\href{https://angms.science/doc/CVX/CVX_HBM.pdf}{Gradient Descent convergence for convex quadratic functions}] \label{thm:quad_gd_convergence}
...
\begin{theorem}[\href{http://www.princeton.edu/~yc5/ele522_optimization/lectures/accelerated_gradient.pdf}{Nesterov's Accelerated Gradient Descent convergence for convex functions}] \label{thm:cvx_nag_convergence}
...
\begin{theorem}[\href{http://www.princeton.edu/~yc5/ele522_optimization/lectures/accelerated_gradient.pdf}{Nesterov's Accelerated Gradient Descent convergence for strongly convex functions}] \label{thm:str_cvx_nag_convergence}
Let $f: \Re^m \rightarrow \Re$ be a L-Lipschitz continuous and $\mu$-strongly convex function. Then the Nesterov's Accelerated Gradient Descent with step size $\alpha \leq 1/L$ and momentum $\beta = \displaystyle \frac{1 - \sqrt{\mu / L}}{1 + \sqrt{\mu / L}} = \frac{1-1/\sqrt{\kappa}}{1+1/\sqrt{\kappa}}$ satisfies:
$$
\begin{aligned}
...
Several losses and objectives have been presented in section~\ref{section:svc} and~\ref{section:svr}. In our experiments, we will consider four different convex loss functions, two for the \emph{classification} and two for the \emph{regression} tasks. In particular, two
of them are nonsmooth convex functions, i.e., the \emph{hinge} and the \emph{epsilon-insensitive} losses for \emph{classification} and \emph{regression} tasks respectively, that linearly penalizes the misclassified points, i.e., $\mathcal{L}_1$-SVM, meanwhile, their two \emph{squared} versions are smooth, i.e., $\mathcal{L}_2$-SVM, that quadratically penalizes the misclassified points.
...
objective & 		& 		\\
\midrule
$\mathcal{L}_1$-SVC~\eqref{eq:reg_bias_primal_l1_svc} & locally & convex \\
$\mathcal{L}_2$-SVC~\eqref{eq:reg_bias_primal_l2_svc} & globally & \vtop{\hbox{\strut 1-strongly}\hbox{\strut convex}} \\
\midrule
$\mathcal{L}_1$-SVR~\eqref{eq:reg_bias_primal_l1_svr} & locally & convex \\
$\mathcal{L}_2$-SVR~\eqref{eq:reg_bias_primal_l2_svr} & globally & \vtop{\hbox{\strut 1-strongly}\hbox{\strut convex}} \\
\bottomrule
\end{tabular}
\end{table}
...
$\mathcal{L}_2$-SVM~(\ref{eq:reg_bias_primal_l2_svc},~\ref{eq:reg_bias_primal_l2_svr}) & $\displaystyle \mathcal{O}\Bigg(\frac{1}{t}\Bigg)$ & $\displaystyle \mathcal{O}\Bigg(\frac{1}{t}\Bigg)$ & $\displaystyle \mathcal{O}\Bigg(\exp\Bigg(-\frac{t}{\sqrt{\kappa}}\Bigg)\Bigg)$ \\
\bottomrule
\end{tabular}
\end{table}
