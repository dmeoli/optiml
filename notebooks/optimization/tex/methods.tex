...
For the GD method we can write the \emph{iteration complexity}, i.e., the smallest $t$ such that we’re within $\epsilon$-close to global optimum, as $\displaystyle \mathcal{O}\Big(\frac{1}{\epsilon}\Big)$ for L-Lipschitz continuous and convex functions and as $\displaystyle \mathcal{O}\Big(\kappa \log \frac{1}{\epsilon}\Big)$ for L-Lipschitz continuous and strongly convex functions.
