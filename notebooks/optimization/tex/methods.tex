...
$$
f(x_t) - f(x_*) \leq \frac{\| x_0 - x_* \|^2}{2 \alpha k}
$$
...
Let $f: \Re^m \rightarrow \Re$ be a L-smooth, $\mu$ strictly convex function. Let $x_*$ be the minimum of $f$ on $\Re^m$. Then the Gradient Descent with step size $\alpha \leq 1/L$ satisfies:
$$
f(x_t) - f(x_*) \leq (1 - \alpha \mu)^t \| x_0 - x_* \|^2
$$
In particular, for $\alpha = 1/L$:
$$
(1 - \frac{\mu}{L})^t \| x_0 - x_* \|^2
$$
\end{theorem}
...
Let $f: \Re^m \rightarrow \Re$ be a L-smooth convex function. Let $x_*$ be the minimum of $f$ on $\Re^m$. Then the Gradient Descent with step size $\alpha \leq 1/L$ and $\beta_{k+1} = k / (k+3)$ satisfies:
$$
f(x_t) - f(x_*) \leq \frac{2 \| x_0 - x_* \|^2}{\alpha (k+1)^2}
$$
In particular, for $\alpha = 1/L$:
$$
\frac{2L \| x_0 - x_* \|^2}{(k+1)^2}
$$
\end{theorem}

\begin{theorem}[Nesterov Accelerated Gradient Descent convergence for strictly convex functions] \label{thm:str_cvx_nag_convergence}
Let $f: \Re^m \rightarrow \Re$ be a L-smooth, $\mu$ strictly convex function. Let $x_*$ be the minimum of $f$ on $\Re^m$. Then the Gradient Descent with step size $\alpha \leq 1/L$ and $\beta_t = \displaystyle \frac{1 - \sqrt{\mu / L}}{1 + \sqrt{\mu / L}}$ satisfies:
$$
f(x_t) - f(x_*) \leq \frac{\| x_0 - x_* \|^2}{\alpha} (1 - \sqrt{\frac{\mu}{L}})^t
$$
In particular, for $\alpha = 1/L$:
$$
f(x_t) - f(x_*) \leq L \| x_0 - x_* \|^2 (1 - \sqrt{\frac{\mu}{L}})^t
$$
\end{theorem}
