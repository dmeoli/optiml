...
\begin{definition}[$L_f$-Lipschitz continuity for convex functions] \label{def:l_f_lipschitz_cvx_continuity}
...
Also, both the \emph{margin-based} losses, i.e., the \emph{hinge} and the \emph{squared hinge} losses, are $L_f$-Lipschitz continuous; meanwhile, among the \emph{distance-based} losses, the \emph{epsilon-insensitive} loss is $L_f$-Lipschitz continuous but the \emph{squared epsilon-insensitive} is not $L_f$-Lipschitz continuous, however it is convex and for this reason is locally $L_f$-Lipschitz continuous. Also the regularization term, i.e., $\displaystyle \frac{1}{2} \| w \|^2$, is not $L_f$-Lipschitz continuous since it becomes arbitrarily steep as $w$ approaches infinity, but it is strictly convex and for this reason is locally $L_f$-Lipschitz continuous.
...
objective & 		& 		& 		\\
\midrule
$\mathcal{L}_1$-SVC~\eqref{eq:reg_bias_primal_l1_svc} & no & \vtop{\hbox{\strut $L_f$-Lipschitz}\hbox{\strut on $K=[0,1]$}} & convex \\
...
\midrule
$\mathcal{L}_1$-SVR~\eqref{eq:reg_bias_primal_l1_svr} & no & \vtop{\hbox{\strut $L_f$-Lipschitz}\hbox{\strut on $K=[0,1]$}} & convex \\
...
objective & 		\\
\midrule
$\mathcal{L}_1$-SVM~(\ref{eq:svc_lagrangian_dual},~\ref{eq:svr_lagrangian_dual}) or~(\ref{eq:svc_bcqp_lagrangian_dual},~\ref{eq:svr_bcqp_lagrangian_dual}) & $\displaystyle \mathcal{O}\Bigg(\frac{nm}{\sqrt{t}}\Bigg)$ \\
% $\mathcal{L}_2$-SVM~(\ref{eq:l2_svc_lagrangian_dual},~\ref{eq:reg_bias_wolfe_dual_l2_svc}) or~(\ref{eq:l2_svr_lagrangian_dual},~\ref{eq:reg_bias_wolfe_dual_l2_svr}) & $\displaystyle \mathcal{O}\Bigg(\frac{nm}{t}\Bigg)$ \\
\bottomrule
\end{tabular}
\end{table}
