...
\begin{definition}[Convexity] \label{def:convexity}
	...
	\begin{enumerate}[(i)]
		...
		$$ i.e., the Hessian matrix is \emph{positive semidefinite}.
	\end{enumerate}
\end{definition}
...
\begin{definition}[Strict Convexity] \label{def:strict_convexity}
	...
	\begin{enumerate}[(i)]
		...
		$$ i.e., the Hessian matrix is \emph{positive definite}.
	\end{enumerate}
\end{definition}
...
$$
i.e., the Hessian matrix is \emph{positive definite}, which is:
$$
...
Strong convexity and L-Lipschitz continuity are related by Fenchel duality according to the following theorem, whose proof is given in~\cite{zhou2018fenchel}.
...
Unfortunately, Polyak’s stepsize rule requires knowledge of $f(x^*)$, which is often unknown a priori, so we might often need a simpler rule for setting stepsizes.
...
Among algorithms that only use subgradient, these \emph{convergence rates} cannot be further improved.
...
At each iteration, SMO chooses two $\alpha_i$ to jointly optimize, let $\alpha_1$ and $\alpha_2$, finds the optimal values for these multipliers and updates the SVM to reflect these new values. In order to solve for two Lagrange multipliers, SMO first computes the constraints over these and then solves for the constrained minimum. Since there are only two multipliers, the box-constraints cause the Lagrange multipliers to lie within a box, while the linear equality constraint causes the Lagrange multipliers to lie on a diagonal line inside the box. So, the constrained minimum must lie there as shown in~\ref{fig:smo_lagrange_multipliers}.
...
In case of classification the ends of the diagonal line segment, i.e., the lower and upper bounds, can be expressed as follows if the target $y_1 \ne y_2$:
...
that will be greater than zero if the kernel matrix will be positive definite, so there will be a minimum along the linear equality constraints that will be:
...
the steps described above will be iterated as long as there will be an example that violates them.
...
so, the steps described above will be iterated as long as there will be an example that violates them.
...
Due to the sparsity of the weight vector of the \emph{Lagrangian dual}, i.e., the Lagrange multipliers, we might end up in a situation where some components of the gradient are very small and others large. This, in terms of \emph{conditioning number}, i.e., $\kappa = L/\mu \gg 1$, means that the level sets of $f$ are ellipsoids, i.e., we are dealing with an ill-conditioned problem. So, given a learning rate, a standard gradient descent approach might end up in a situation where it decreases too quickly the small weights or too slowly the large ones.
...
The above suggests a general rule in optimization: find any preconditioner, in convex optimization it has to be positive semidefinite, that improves the performance of gradient descent in terms of iterations, but without wasting too much time to compute that preconditioner. The above results in:
...
The \emph{AdaGrad}~\cite{duchi2011adaptive} algorithm is just a variant of preconditioned gradient descent, where $P$ is selected to be a diagonal preconditioner matrix and is updated using the gradient information, in particular it is the diagonal approximation of the inverse of the square roots of gradient outer products, until the $k$-th iteration. The above leads to the algorithm:
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		\Require{Function $f$ to minimize}
		\Require{Learning rate or step size $\alpha > 0$}
		\Require{Offset $\epsilon > 0$ to ensure not divide by 0}
