...
\begin{equation} \label{eq:hinge1}
	\mathcal{L}_1 = 
	\begin{cases}
...
\begin{equation} \label{eq:hinge2}
...
\begin{equation} \label{eq:hinge_der}
...
\begin{equation} \label{eq:augmented_w_b}
	\begin{aligned}
		& w^T = [w^T, b] \\
		& x_i^T = [x_i^T, 1]
	\end{aligned}
...
\begin{equation} \label{eq:primal_svc_hinge}
    \min_{w} \frac{1}{2} \Vert w \Vert^2 + C \sum_{i=1}^n \max(0, 1 - y_i (w^T x_i))
\end{equation}
...
Since smoothed versions of objective functions may be preferred for optimization, we can reformulate the \ref{eq:primal_svc_hinge} as:

\begin{equation} \label{eq:primal_svc_sq_hinge}
    \min_{w} \frac{1}{2} \Vert w \Vert^2 + C \sum_{i=1}^n \max(0, 1 - y_i (w^T x_i))^2
\end{equation}
