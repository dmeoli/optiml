...
In case of regression the lower and upper bounds, the new multipliers $\alpha_1$ and $\alpha_2$ can be espressed as follow if $\alpha_1^+ > 0$ and $\alpha_2^+ > 0$:
...
\begin{equation} \label{eq:smo_svr_b1}
	b_1 = E_1 + (\alpha_1^{new} - \alpha_1) K(x_1,x_1) + (\alpha_2^{new,clipped} - \alpha_2) K(x_1,x_2) + b
\end{equation}

\begin{equation} \label{eq:smo_svr_b2}
	b_2 = E_2 + (\alpha_1^{new} - \alpha_1) K(x_1,x_2) + (\alpha_2^{new,clipped} - \alpha_2) K(x_2,x_2) + b
\end{equation}
