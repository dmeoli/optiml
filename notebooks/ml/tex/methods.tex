...
\begin{breakablealgorithm}
	...
	\begin{algorithmic}
		...
		\Function{SMOClassifier}{$X,y,K,C,tol$}
			...
			\State Initialize the set $I1 \gets \{i : y_i = +1, \alpha_i = 0\}$ to contain all training examples
			\State Initialize the empty set $I2 \gets \{i : y_i = -1,	 \alpha_i = C\}$
			\State Initialize the empty set $I3 \gets \{i :  y_i = +1,	 \alpha_i = C\}$
			\State Initialize the empty set $I4 \gets \{i : y_i = -1, \alpha_i = 0\}$
			...
			\While {$num\_changed > 0$ \OR $examine\_all = True$}
				...
				\If {$examine\_all = True$}
					...
					\EndFor
				\Else
					\For {$i$ in $I0$} \Comment loop over examples where $\alpha$ are not already at their bounds
						\State $num\_changed \gets num\_changed + \Call{ExamineExample}{i}$
	...
	\begin{algorithmic}
		\Require{First Lagrange multiplier $i1$}
		\Require{Second Lagrange multiplier $i2$}
		\Function{TakeStep}{$i1,i2$}
			...
			\If {changes in $\alpha_2^{new,clipped}$ and $\alpha_1^{new}$ are larger than some eps}
			...
			\If {changes in $\alpha_2^{new,clipped}$ and $\alpha_1^{new}$ are larger than some eps}
				\For {$i$ in $I0$}
					...
					\State Update $errors_i1$ and $errors_i2$
					\For {$i$ in $I0 \cup \{i1,i2\}$}
						\State Compute $(i_{low}, b_{low})$ and $(i_{up}, b_{up})$ using $b_{low} = max{errors_i : i \in I0 \cup I3 \cup I4}$ and $b_{up} = min{errors_i : i \in I0 \cup I1 \cup I2}$ respectively
					\EndFor
				\EndFor
				\State \Return True
			\Else
...
\begin{breakablealgorithm}
		...
		\Function{SMORegression}{$X,y,K,C,\epsilon,tol$}
			...
			\While {$num\_changed > 0$ \OR $examine\_all = True$}
				...
				\If {$examine\_all = True$}
					...
					\EndFor
				\Else
					\For {$i$ in $I0$} \Comment loop over examples where $\alpha^+$ and $\alpha^-$ are not already at their bounds
						\State $num\_changed \gets num\_changed + \Call{ExamineExample}{i}$
						\If {$b_{up} > b_{low} - 2 tol$} \Comment check if optimality on $I0$ is attained
							\State {$num\_changed \gets 0$}
							\Break
	...
	\begin{algorithmic}
		\Require{Training example $i2$}
		\Function{ExamineExample}{$i2$}
			\If {$i2 \in I0$}
				\State $E_2 \gets errors_{i2}$
			\Else 
				\State $E_2 \gets y_{i2} - (\alpha^+ - \alpha^-) \odot K_{i2}$
	...
	\begin{algorithmic}
		...
		\Function{TakeStep}{$i1,i2$}
			...
			\If {$L < H$}
				\State Compute $\alpha_2^{+,new}$ and $\alpha_2^{-,new}$ using \eqref{eq:smo_svr_a2_new1}, \eqref{eq:smo_svr_a2_new3} or \eqref{eq:smo_svr_a2_new2}, \eqref{eq:smo_svr_a2_new1} respectively
				\State Compute $\alpha_2^{+,new,clipped}$ and $\alpha_2^{-,new,clipped}$ by \eqref{eq:smo_svc_a2_new_clipped}
				\State Compute $\alpha_1^{+,new}$ and $\alpha_1^{-,new}$ using \eqref{eq:smo_svr_a1_new1}, \eqref{eq:smo_svr_a1_new2} or \eqref{eq:smo_svr_a1_new3}, \eqref{eq:smo_svr_a1_new4} respectively
				\If {changes in $\alpha_2^{+,new,clipped}, \alpha_2^{-,new,clipped}, \alpha_1^{+,new}$ and $\alpha_1^{-,new}$ are larger than some eps}
					\State update $\alpha_2^{+,new,clipped}, \alpha_2^{-,new,clipped}, \alpha_1^{+,new}$ and $\alpha_1^{-,new}$
				\EndIf
			\EndIf
			\If {changes in $\alpha_2^{+,new,clipped}, \alpha_2^{-,new,clipped}, \alpha_1^{+,new}$ and $\alpha_1^{-,new}$ are larger than some eps}
				\For {$i$ in $I0$}
					\State Update $errors_i$ using new Lagrange multipliers
					\State Update $\alpha^+$ and $\alpha^-$ using the new Lagrange multipliers
					\State Update $I0, I1, I2$ and $I3$
					\State Update $errors_i1$ and $errors_i2$
					\For {$i$ in $I0 \cup \{i1,i2\}$}
						\State Compute $(i_{low}, b_{low})$ and $(i_{up}, b_{up})$
					\EndFor
				\EndFor
				\State \Return True
			\Else
