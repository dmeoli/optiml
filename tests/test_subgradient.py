...
def test_quadratic():
    x, status = Subgradient(gen_quad_1).minimize()
    assert np.allclose(x, gen_quad_1.x_star)
    assert status is 'optimal'

    x, status = Subgradient(gen_quad_2).minimize()
    assert np.allclose(x, gen_quad_2.x_star)
    assert status is 'optimal'

    # x, status = Subgradient(gen_quad_3).minimize()
    # assert np.allclose(x, gen_quad_3.x_star)
    # assert status is 'optimal'

    # x, status = Subgradient(gen_quad_4).minimize()
    # assert np.allclose(x, gen_quad_4.x_star)
    # assert status is 'optimal'

    x, status = Subgradient(gen_quad_5).minimize()
...
def test_Rosenbrock():
    ...
    x, status = Subgradient(obj).minimize()
