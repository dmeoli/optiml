...
def test_quadratic():
    x, _ = Subgradient(gen_quad_1).minimize()
    assert np.allclose(gen_quad_1.jacobian(x), 0)

    x, _ = Subgradient(gen_quad_2).minimize()
    assert np.allclose(gen_quad_2.jacobian(x), 0)

    # x, _ = Subgradient(gen_quad_3).minimize()
    # assert np.allclose(gen_quad_3.jacobian(x), 0)

    # x, _ = Subgradient(gen_quad_4).minimize()
    # assert np.allclose(gen_quad_4.jacobian(x), 0)

    x, _ = Subgradient(gen_quad_5).minimize()
