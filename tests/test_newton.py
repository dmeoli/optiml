...
def test_quadratic_functions():
    ...
    # TODO
    x, status = NWTN(gen_quad_3, x0)
    assert np.allclose(x, [[-7.61717505e+14], [-7.61717505e+14]])
    assert status is 'stopped'

    # TODO
    x, status = NWTN(gen_quad_4, x0)
    assert np.allclose(x, [[-3.86184022e+52], [-3.86184079e+52]])
    assert status is 'stopped'
