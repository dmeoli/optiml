...
def test_RProp_quadratic():
    assert np.allclose(RProp(f=quad1).minimize().x, quad1.x_star())
    assert np.allclose(RProp(f=quad2).minimize().x, quad2.x_star())


def test_RProp_Rosenbrock():
    rosen = Rosenbrock()
    assert np.allclose(RProp(f=rosen).minimize().x, rosen.x_star(), rtol=0.1)
