...
def test_solve_primal_l1_svc_with_line_search_optimizers():
    ...
    svc = OVR(PrimalSVC(loss=hinge, optimizer=HeavyBallGradient))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.57
...
def test_solve_dual_l1_svc_with_unreg_intercept_with_bcqp_optimizers():
    ...
    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=ProjectedGradient, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97

    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=ActiveSet, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97

    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=InteriorPoint, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97

    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=FrankWolfe, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97
...
def test_solve_dual_l1_svc_with_reg_intercept_with_line_search_optimizers():
    ...
    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=Subgradient, reg_intercept=True))
    svc = svc.fit(X_train, y_train)
    ...
    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=HeavyBallGradient, reg_intercept=True))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97

    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=Newton, reg_intercept=True))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97

    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=BFGS, reg_intercept=True))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97
...
def test_solve_dual_l1_svc_with_unreg_intercept_with_line_search_optimizers():
    ...
    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=Subgradient, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.92
    ...
    svc = OVR(DualSVC(loss=hinge, kernel=gaussian, optimizer=HeavyBallGradient, max_iter=2000, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97
...
def test_solve_primal_l2_svc_with_line_search_optimizers():
    ...
    svc = OVR(PrimalSVC(loss=squared_hinge, optimizer=HeavyBallGradient))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.57
...
def test_solve_dual_l2_svc_with_reg_intercept_with_line_search_optimizers():
    ...
    svc = OVR(DualSVC(loss=squared_hinge, kernel=gaussian, optimizer=HeavyBallGradient, reg_intercept=True))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97
...
def test_solve_dual_l2_svc_with_unreg_intercept_with_line_search_optimizers():
    ...
    svc = OVR(DualSVC(loss=squared_hinge, kernel=gaussian, optimizer=HeavyBallGradient, reg_intercept=False))
    svc = svc.fit(X_train, y_train)
    assert svc.score(X_test, y_test) >= 0.97
