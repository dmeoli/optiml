...
from optiml.optimization.unconstrained.stochastic import AdaGrad, StochasticGradientDescent
...
#     X_scaled = StandardScaler().fit_transform(X)
#     X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, train_size=0.75, random_state=1)
#     svr = DualSVR(kernel=linear, optimizer=AdaGrad, learning_rate=0.01, max_iter=1000)
#     svr.fit(X_train, y_train)
#     assert svr.score(X_test, y_test) >= 0.75
...
#     X_scaled = MinMaxScaler().fit_transform(X)
#     X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, train_size=0.75, random_state=1)
#     svc = OneVsRestClassifier(DualSVC(kernel=rbf, optimizer=AdaGrad, learning_rate=0.01, max_iter=1000))
#     svc.fit(X_train, y_train)
#     assert svc.score(X_test, y_test) >= 0.95
