...
class Conv2D(ParamLayer):
    def __init__(self, n_in, n_out, kernel_size=(3, 3), strides=(1, 1), padding='valid',
                 channels_last=True, activation=None, w_init=None, b_init=None, use_bias=True):
        self.kernel_size = get_tuple(kernel_size)
        self.strides = get_tuple(strides)
        super().__init__((n_in,) + self.kernel_size + (n_out,), activation, w_init, b_init, use_bias)
