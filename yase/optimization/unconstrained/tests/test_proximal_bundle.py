...
def test_quadratic():
    ...
    assert np.allclose(ProximalBundle(quad2).minimize()[0], quad2.x_star(), rtol=0.1)
...
def test_standard_momentum_quadratic():
    assert np.allclose(ProximalBundle(quad1, momentum_type='standard').minimize()[0], quad1.x_star(), rtol=0.1)
    assert np.allclose(ProximalBundle(quad2, momentum_type='standard').minimize()[0], quad2.x_star(), rtol=0.1)
...
def test_Nesterov_momentum_quadratic():
    assert np.allclose(ProximalBundle(quad1, momentum_type='nesterov', momentum=0.2).minimize()[0],
                       quad1.x_star(), rtol=0.1)
    assert np.allclose(ProximalBundle(quad2, momentum_type='nesterov', momentum=0.2).minimize()[0],
