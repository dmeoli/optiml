...
if __name__ == '__main__':
    ...
    #    solution anche con il metodo di newton o bfgs
    #  - fixare la gestione dei batch negli algoritmi stocastici e stampare loss, val_loss, acc, val_acc
    #  - implementare early stopping come in sklearn
    #  - fixare gli initializers passando i random state dalla neural network o degli optimizers oppure
    #    come in scipy e rimuovere l'inizializzazione randomica
    ...
    cnn = NeuralNetworkClassifier(
        (Conv2D(in_channels=1, out_channels=6, kernel_size=(5, 5), strides=(1, 1),
                padding='same', channels_last=True, activation=linear),  # => [n,28,28,6]
         MaxPool2D(pool_size=(2, 2), strides=(2, 2)),  # => [n,14,14,6]
         Conv2D(in_channels=6, out_channels=16, kernel_size=(5, 5), strides=(1, 1),
                padding='same', channels_last=True, activation=linear),  # => [n,14,14,16]
        ...
        max_iter=300, batch_size=64, verbose=1)
